...
sys.path.append("./calculating_model_score/")
...
flags.DEFINE_integer(
    "max_seq_length", 128,
    "The maximum total input setquence length after WordPiece tokenization. "
...
class Snips_Slot_Filling_and_Intent_Detection_Processor(DataProcessor):
    ...
    def get_slot_labels(self):
        ...
        config_path = os.path.join(root_dir,"config_1.ini")
        cf = configparser.ConfigParser()
        cf.read(config_path,encoding="UTF-8")
        slots_list = cf.get(FLAGS.source_task_num, "slots_labels")
        slots_labels = json.loads(slots_list)
        return slots_labels
    ...
    def get_intent_labels(self):
        ...
        cf = configparser.ConfigParser()
        cf.read(config_path,encoding="UTF-8")
        intent_list = cf.get(FLAGS.source_task_num, "intent_labels")
        intent_labels = json.loads(intent_list)
...
def model_fn_builder(bert_config, num_slot_labels, num_intent_labels, init_checkpoint,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                predictions={"intent_logits":intent_logits,
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        if int(len(train_examples) / FLAGS.train_batch_size * FLAGS.num_train_epochs)==0:
            num_train_steps = 1
        else:
            num_train_steps = int(
    ...
    if FLAGS.do_predict:
        ...
        intent_logits_output_predict_file = os.path.join(FLAGS.output_dir, "intent_logits.txt")
        slot_logits_output_predict_file = os.path.join(FLAGS.output_dir, "slot_logits.txt")
        with tf.gfile.GFile(intent_logits_output_predict_file, "w") as intent_logits_writer:
            with tf.gfile.GFile(slot_logits_output_predict_file, "w") as slot_logits_writer:
                with tf.gfile.GFile(intent_output_predict_file, "w") as intent_writer:
                    with tf.gfile.GFile(slot_output_predict_file, "w") as slot_writer:
                        ...
                        for (i, prediction) in enumerate(result):
                            intent_logits = prediction["intent_logits"]
                            slot_logits = prediction["slot_logits"]
                            intent_prediction = prediction["intent_predictions"]
                            slot_predictions = prediction["slot_predictions"]
                            if i >= num_actual_predict_examples:
                                break

                            intent_logits_writer.write(str(intent_logits)+ "\n")
                            slot_logits_writer.write(str(slot_logits)+ "\n")
